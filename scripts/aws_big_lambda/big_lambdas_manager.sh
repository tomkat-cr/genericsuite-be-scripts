...
set_env_vars_file() {
    ...
    cd "${REPO_BASEDIR}"
    pwd
    echo "sh ${SCRIPTS_DIR}/../aws/update_config.sh ${STAGE} deploy"
    sh ${SCRIPTS_DIR}/../aws/update_config.sh ${STAGE} deploy
...
export FDA_API_KEY="${FDA_API_KEY}"
...
prepare_tmp_build_dir() {
    ...
    echo ""
    echo "Prepare temporary build directory ended."
}
...
create_sam_yaml() {
  ...
  if [ "${DOMAIN_NAME}" = "" ];then
    ...
    perl -i -pe "s|CertificateArn: CertificateArn_placeholder|CertificateArn: ${AWS_SSL_CERTIFICATE_ARN}|g" "./template.yml"
  fi        
  ...
  perl -i -pe "s|FDA_API_KEY:.*|FDA_API_KEY: ${FDA_API_KEY}|g" "./template.yml"
...
REPO_BASEDIR="`pwd`"
# SCRIPTS_DIR="${REPO_BASEDIR}/scripts/aws_big_lambda"
cd "`dirname "$0"`"
SCRIPTS_DIR="`pwd`"
cd "${REPO_BASEDIR}"
...
AWS_API_GATEWAY_NAME="${AWS_LAMBDA_FUNCTION_NAME}"
AWS_STACK_NAME="${AWS_LAMBDA_FUNCTION_NAME}"
MEMORY_SIZE="512"
ARCHITECTURES="arm64"
