...
export HUGGINGFACE_TEXT_TO_IMAGE_ENDPOINT="${HUGGINGFACE_TEXT_TO_IMAGE_ENDPOINT}"
...
END
  ...
  HUGGINGFACE_TEXT_TO_IMAGE_ENDPOINT="${HUGGINGFACE_TEXT_TO_IMAGE_ENDPOINT}"
  ...
  --env HUGGINGFACE_TEXT_TO_IMAGE_ENDPOINT=\"${HUGGINGFACE_TEXT_TO_IMAGE_ENDPOINT}\"
...
create_sam_yaml() {
  ...
  perl -i -pe"s|HUGGINGFACE_TEXT_TO_IMAGE_ENDPOINT_placeholder|${HUGGINGFACE_TEXT_TO_IMAGE_ENDPOINT}|g" "${TMP_WORKING_DIR}/template.yml"
...
verify_requirements_with_local_dependencies() {
    ...
    if [ "${LOCAL_DEPENDENCIES_ERROR}" = "1" ]; then
        ...
        echo "   or"
        echo "   ${PEM_TOOL} install git+https://github.com/tomkat-cr/genericsuite-be-ai@branch_name"
