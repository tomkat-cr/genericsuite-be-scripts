...
if [ "$1" = "pipfile" ]; then
    ...
    # Fix temporary situation with the "genericsuite_ai" install directly from the git repo:
    perl -i -pe "s|genericsuite@.*|genericsuite@ git+https:\/\/github.com\/tomkat-cr\/genericsuite-be|g" "${REPO_BASEDIR}/requirements.txt"
    perl -i -pe "s|genericsuite-ai@.*|genericsuite-ai@ git+https:\/\/github.com\/tomkat-cr\/genericsuite-be-ai|g" "${REPO_BASEDIR}/requirements.txt"
fi
...
if [[ "$1" = "run_local" || "$1" = "" ]]; then
    ...
    export IP_ADDRESS=$(sh ${REPO_BASEDIR}/scripts/get_localhost_ip.sh)
    ...
    if [ "${CURRENT_FRAMEWORK}" = "chalice" ]; then
        echo "Run over: 1) https [chalice_docker], 2) http [chalice], 3) use current [${RUN_METHOD}] (1/2/3) ?"
        read choice
        while [[ ! $choice =~ ^[123]$ ]]; do
    ...
    if [ "${RUN_METHOD}" = "chalice" ]; then
        echo "sh ${SCRIPTS_DIR}/update_config.sh ${STAGE}" http
        sh ${SCRIPTS_DIR}/update_config.sh ${STAGE} http
        echo ""
    ...
    if [ "${RUN_METHOD}" = "chalice_docker" ]; then
        echo "scripts/secure_local_server/run.sh"
        scripts/secure_local_server/run.sh "run" ${STAGE}
    fi
