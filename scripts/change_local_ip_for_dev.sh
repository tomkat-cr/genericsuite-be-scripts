...
if [ "$#" -ne 1 ]; then
    echo "Usage: $0 IP_ADDRESS_or_DOMAIN [BACKEND_LOCAL_PORT] [FRONTEND_LOCAL_PORT]"
    exit 1
fi
...
#
perl -i -pe "s|APP_CORS_ORIGIN_QA=.*|APP_CORS_ORIGIN_QA=${HTTP_HTTPS}://${IP_ADDRESS_or_DOMAIN}:${FRONTEND_LOCAL_PORT}|g" "${BACKEND_BASE_DIR}/.env"
perl -i -pe "s|APP_CORS_ORIGIN_QA_LOCAL=.*|APP_CORS_ORIGIN_QA_LOCAL=${HTTP_HTTPS}://${IP_ADDRESS_or_DOMAIN}:${FRONTEND_LOCAL_PORT}|g" ".env"
#
# In the frontend:
# REACT_APP_API_URL=http://127.0.0.1:5001
# REACT_APP_API_URL_DEV=http://127.0.0.1:5001
#
perl -i -pe "s|REACT_APP_API_URL=.*|REACT_APP_API_URL=${HTTP_HTTPS}://${IP_ADDRESS_or_DOMAIN}:${BACKEND_LOCAL_PORT}|g" "${FRONTEND_PATH}/.env"
perl -i -pe "s|REACT_APP_API_URL_DEV=.*|REACT_APP_API_URL_DEV=${HTTP_HTTPS}://${IP_ADDRESS_or_DOMAIN}:${BACKEND_LOCAL_PORT}|g" "${FRONTEND_PATH}/.env"
